...
|:---|:---|:---|:---:|:---:|
|**HU-01**| Como **Cajero**, quiero registrar un cliente natural con sus datos personales para apertura de una cuenta | -El sistema valida que el documento de identidad no este repetido - Lanza CapacidadExcedidadException si el banco alcanzo su limite de clienetes. - Datos nulos o vacios lanza DatosInvalidoException | **Alta**| 5 |
|**HU-02**| Como **Cajero**, quiero registrar un cliente empresarial con NIT y razon social para asociarle cuentas bancarias | - El NIT no puede estar duplicado en el sistema - La razon social y representante legal son obligatorios - Lanza CapacidadExcedidaException si se supera el limite | **Alta** | 5 |
|**HU-03**| Como **Cajero**, quiero abrir cuentas de ahorros, corrientes y creditos para los clientes registrados en el sistema | - El cliente debe estar registado en el sistema, de lo contrario lanza un ClienteNoEncontradoException. - Cada cliente puede tener un maximo de 5 cuentas. - Laza CapacidadExcedidaException si se supera el limite. | **Alta** | 8  |
|**HU-04**| Como **Cajero**, quiero procesar depositos en cuentas activas para registrar el ingreso de dinero | El monto debe ser mayo a 0, de lo contrario lanza DatoInvalidoException - Lanza CuentaBloqueadaException si la cuenta esta bloqueada - El saldo se actualiza correctamente tras el desposito  | **Alta** | 5 |
|**HU-05**| Como **Cajero**, quiero procesar retiros validando el saldo disponible para garantizar operaciones seguras | -Lanza SaldoInsuficiente si el monto supera el saldo disponible, indicando saldo actual y monto solicitado - Lanza CuentaBloqueadaException si la cuenta esta bloqueada - Se aplica comision automaticamente segun el tipo de cuenta | **Alta** | 8 |
|**HU-06**| Como **Cajero**, quiero gerenar un comprobante de cada transaccion para entregarsela al cliente | -El comprobante incluye: ID, fecha, cuentas,monto, estado y descripcion. - Se genera tras completar un desposito, retiro o transferecia. | **Medio** | 3 |
|**HU-07**| Como **Asesor Financiero**, quiero realizar trasnferencias entre cuentas para mover fondos entre clientes | -Verifica saldo disponible en la cuenta de origen anres de trasnferir dinero - Lanza SaldoInsuficienteException si no hay fondos suficiente - Lanza CuentaBloqueadaException si la cuenta se encuentra bloqueada | **Alta** | 8 |
|**HU-08**| Como **Asesor Financiero**, quiero consultar la informacion de un  cliente para brindar asesoria personalizada | - Busca el cliente por ID lanza ClienteNoEncontradoException si no existe - Muestra resumen con datos personales, cuentas asociadas y estado | **Medio** | 3 |
|**HU-09**| Como **Asesor Financiero**, quiero cambiar el estado de una transaccion respetando las transacciones validas del sistema | - Transacciones validas: PENDIENTE - PROCESANDO - COMPLETADA- REVERTIDA -Una transaccion invalida lanza EstadoTransaccionInvalidoException con estado origen y destino. - Los estado RECHAZADA y REVERTIDA son finales | **Alta** | 5 |
|**HU-10**| Como **Asesor Financiero**, quiero asignar un cliente a mi cartera para hacer seguimiento personalizado | -Un asesor puede tener maximo 20  clientes asignados - Lanza CapacidadExcedidaException si se supera el limite  | **Medio** | 3 |
|**HU-11**| Como **Gerente de sucursal**, quiero aprobar creditos para los clientes que cumplan los requisitos | - Solo un Gerente puede aprobar creditos. - Si lo intenta otro lanza PermisoInsuficienteException con la accion denegada | **Alta** | 8 |
|**HU-12**| Como **Gerente de sucursal**, quiero calcular la nomina total del banco para controlar el gasto de personal | - Recorre el array de empleados usando polimorfismo sobre calcularSalarioBase() -  Cada tipo de empleado aplica su formula: Salario base + bono correspondiente | **Alta** | 5 |
|**HU-13**| Como **Gerente de sucursal**, quiero registrar nuevos empleados | - El banco puede tener maximo 50 empleados - Lanza CapacidadExcedidadException si se supera el limite - Datos nulos o invalidos lanza DatoInvalidoException | **Alta** | 5 |
|**HU-14**| Como **Gerente de sucursal**, quiero calcular los intereses mensuales de todas las cuentas | - Recorre el array de cuentas usando polimosfirmo sobre calcularInteres() - CuentaAhorros: saldo*tasa/12 - CuentaCredito: deudaActual * tasa / 12 - Cuentacorriente: no genera interes | **Alta** | 5 | 
